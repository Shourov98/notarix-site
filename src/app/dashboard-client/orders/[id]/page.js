...
export default function ClientOrderDetailPage() {
  ...
  useEffect(() => {
    ...
    const loadOrder = async () => {
      ...
      try {
        const payload = await requestPortalJson(`/site/client/orders/${id}`);
        if (!cancelled) {
          setOrder(payload);
        }
      } catch (loadError) {
      ...
      <div className="rounded-[2rem] border border-rose-200 bg-rose-50 p-8 text-rose-700">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-2 text-sm leading-7">{error || "The requested order could not be found."}</p>
          </div>
        </div>
      </div>
    );
    ...
    <div className="space-y-8">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm leading-7 text-zinc-500">
            This screen is rendered entirely from the backend order record.
          </p>
        </div>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="rounded-[2rem] border border-zinc-100 bg-white p-6 shadow-sm">
          <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-400">Signer</p>
        ...
        <div className="rounded-[2rem] border border-zinc-100 bg-white p-6 shadow-sm">
          ...
          <div className="mt-3 flex items-center gap-2 text-sm text-zinc-600">
            <CalendarDays className="h-4 w-4 text-[#1a4fdb]" />
            {formatDate(order.signingDate)}
          </div>
          <div className="mt-2 flex items-center gap-2 text-sm text-zinc-600">
            <Clock3 className="h-4 w-4 text-[#1a4fdb]" />
            {order.signingTime || "Time pending"}
          </div>
          <div className="mt-3 flex items-center gap-2 text-sm text-zinc-600">
            <MapPin className="h-4 w-4 text-[#1a4fdb]" />
        ...
        <div className="rounded-[2rem] border border-zinc-100 bg-white p-6 shadow-sm">
          ...
          <p className="mt-3 text-2xl font-bold text-zinc-900">
            ${Number(order.payment?.feeAmount || 0).toFixed(2)}
          </p>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.35fr_0.9fr]">
        <div className="space-y-6">
          <section className="rounded-[2rem] border border-zinc-100 bg-white p-6 shadow-sm">
            <h2 className="text-xl font-bold text-zinc-900">Order Information</h2>
            ...
            <div className="mt-6 rounded-2xl border border-zinc-100 bg-zinc-50/70 p-4">
              ...
              <p className="mt-2 text-sm leading-7 text-zinc-600">
                {order.specialInstructions || "No special instructions were provided for this order."}
              </p>
            </div>
          </section>
          ...
          <section className="rounded-[2rem] border border-zinc-100 bg-white p-6 shadow-sm">
            ...
            {documents.length ? (
              ...
              <div className="mt-6 rounded-2xl border border-dashed border-zinc-200 bg-zinc-50/50 p-8 text-center">
                ...
                <p className="mt-2 text-sm text-zinc-500">
                  Uploaded order and completed package files will appear here when they exist.
                </p>
              </div>
            )}
          </section>
        ...
        <div className="space-y-6">
          <section className="rounded-[2rem] border border-zinc-100 bg-white p-6 shadow-sm">
            <div className="flex items-center gap-3">
              <ShieldCheck className="h-5 w-5 text-[#1a4fdb]" />
              ...
              <div className="mt-6 space-y-4">
                {order.timeline.map((entry, index) => (
                  <div key={`${entry.status}-${entry.changedAt}-${index}`} className="relative pl-6">
                    ...
                    <div className="absolute left-0 top-1.5 h-5 w-5 rounded-full bg-[#1a4fdb]/10 ring-4 ring-white">
                      <div className="h-5 w-5 rounded-full bg-[#1a4fdb]" />
                    </div>
          ...
          <section className="rounded-[2rem] border border-zinc-100 bg-white p-6 shadow-sm">
            <div className="flex items-center gap-3">
              <CheckCircle2 className="h-5 w-5 text-[#1a4fdb]" />
            ...
            <div className="mt-6 space-y-4 text-sm text-zinc-600">
              ...
              <p>
                ...
                {order.payment?.paymentMethod || "Not set"}
              </p>
              <p>
