    ...
    <div className="min-h-screen bg-white flex flex-col">
      <header className="h-20 px-8 border-b border-zinc-200 flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-700 mt-2">Refinance_Agreement_v4.pdf • Order #{id}</p>
        </div>
        <div className="flex items-center gap-4">
          ...
          <Link href={`/dashboard-notary/assignments-orders/${id}`} className="text-gray-700">
            <X className="w-8 h-8" />
          </Link>
        </div>
      </header>
      ...
      <div className="flex-1 flex">
        ...
        <div className="w-80 border-l border-zinc-200 bg-zinc-50 p-6 space-y-8">
          <div>
            <p className="text-xs uppercase tracking-[0.2em] font-bold text-gray-700">File Details</p>
            <div className="mt-5 flex gap-4">
              <div className="w-12 h-12 rounded-xl bg-zinc-200 flex items-center justify-center text-gray-700">
                <FileText className="w-6 h-6" />
              </div>
              <div>
                <p className="text-3xl font-bold text-zinc-900">Refinance_Agreement_v4</p>
                <p className="text-gray-700 mt-2">2.4 MB • PDF Document</p>
              </div>
            </div>
            <div className="mt-5 bg-white border border-zinc-200 rounded-[24px] p-5 space-y-4">
              <div className="flex justify-between"><span className="text-gray-700">Uploaded By</span><span className="font-bold text-[#2c49df]">Notary</span></div>
              <div className="flex justify-between"><span className="text-gray-700">Date</span><span className="font-semibold">Oct 24, 2023</span></div>
              <div className="flex justify-between"><span className="text-gray-700">Order ID</span><span className="font-semibold">{id}</span></div>
            </div>
          </div>

          <div>
            <p className="text-xs uppercase tracking-[0.2em] font-bold text-gray-700">Signing Status</p>
              ...
              {[
                ...
                <div key={name} className="bg-white border border-zinc-200 rounded-[24px] p-4 flex items-center justify-between gap-4">
                  ...
                  <div className="text-right">
                    <span className={`px-3 py-1 rounded-full text-sm font-bold ${status === "Signed" ? "bg-emerald-50 text-emerald-600" : "bg-zinc-100 text-gray-700"}`}>{status}</span>
                    {time && <p className="text-gray-700 text-sm mt-2">{time}</p>}
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="flex-1 flex flex-col">
            <p className="text-xs uppercase tracking-[0.2em] font-bold text-gray-700">Legal Notes</p>
            <div className="mt-5 flex-1 bg-white border border-zinc-200 rounded-[24px] p-5">
              <p className="text-gray-700">Add a confidential note about this document...</p>
            </div>
      ...
      <footer className="py-6 text-center text-gray-700 border-t border-zinc-100">
