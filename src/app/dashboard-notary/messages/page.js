        ...
        <div className="space-y-6 p-6">
          ...
          <div className="group relative">
            ...
            <input
              ...
              className="w-full rounded-xl border border-zinc-200 bg-white py-2.5 pl-10 pr-4 text-sm focus:border-[#2c49df] focus:outline-none focus:ring-2 focus:ring-[#2c49df]/10"
              disabled
            />
          </div>
        </div>
          ...
          {loadingConversations ? (
              ...
              <button
                ...
                className={`flex w-full gap-3 p-4 text-left transition-colors ${
                  conversation.id === selectedId
                    ? "border-l-4 border-l-[#2c49df] bg-white"
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-1 flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-bold text-[#2c49df]">
            ...
            <div className="px-6 py-16 text-center">
              ...
              <p className="mt-2 text-sm text-zinc-500">
                Conversation threads will appear when real assignment chats exist.
              </p>
            </div>
          )}
        </div>
        ...
        {activeConversation ? (
          <>
            <div className="flex h-20 shrink-0 items-center justify-between border-b border-zinc-100 px-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="mt-1 flex items-center gap-2">
                    <span className="text-xs font-bold text-[#2c49df]">
              ...
              <Link
                href={`/dashboard-notary/assignments-orders/${activeConversation.orderId}`}
                  ...
                  {selectedThread.messages.map((message) => (
                    ...
                    >
                      <div className={`max-w-[80%] ${message.isOwnMessage ? "items-end" : "items-start"} flex flex-col gap-1`}>
                        <span className={`text-[10px] font-bold uppercase tracking-widest ${message.isOwnMessage ? "text-[#2c49df]" : "text-zinc-400"}`}>
                          {message.senderName} • {formatDateTime(message.createdAt)}
                        </span>
                        <div
                          className={`rounded-2xl p-4 text-sm leading-relaxed shadow-sm ${
                            message.isOwnMessage
                              ? "rounded-tr-sm bg-[#2c49df] text-white"
            ...
            <div className="flex shrink-0 items-center gap-3 border-t border-zinc-100 bg-white p-4">
              ...
              <input
                ...
                className="flex-1 rounded-xl border border-zinc-200 px-4 py-3 text-sm transition-all focus:border-[#2c49df] focus:outline-none focus:ring-2 focus:ring-[#2c49df]/10"
              />
              <button
                type="button"
                onClick={sendMessage}
                disabled={sending || !draft.trim()}
                className="flex items-center gap-2 rounded-xl bg-[#2c49df] px-6 py-3 text-sm font-bold text-white shadow-lg shadow-blue-100 transition hover:bg-[#243dcc] disabled:cursor-not-allowed disabled:opacity-70"
              >
          ...
          <div className="flex flex-1 items-center justify-center px-6 text-center">
            <div>
              ...
              <p className="mt-2 text-sm text-zinc-500">
                Select a real conversation from the left, or wait until an assignment chat is created.
              </p>
            </div>
          </div>
        )}
